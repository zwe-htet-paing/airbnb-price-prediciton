...
from mlops.utils.airbnb_data_preparation.encoders import vectorize_features
...
def test_dataset(
    ...
    *args,
    ...
    assert (
        X.shape[0] == 344
    ), f'Entire dataset should have 344 examples, but has {X.shape[0]}'
    assert (
        X.shape[1] == 24
    ), f'Entire dataset should have 9 features, but has {X.shape[1]}'
    assert (
